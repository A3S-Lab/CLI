...
| **Tool** | Verified non-interactive native Task lifecycle, exact-generation Runtime dispatch from durable schema-v4 receipts, and watched TUI projection as conservative `use_tool_*` tools. Projection carries the exact package/manifest digests, lifecycle generation, scope, surface, and provider ID; invocation accepts only bounded argv and never consults current assignments. A trusted Linux host ACL can explicitly assign the shared Box provider to release-backed Tool Tasks; omission and unsupported platforms fail closed without registering a tool. Upgrade and disable withdraw the old generation. | Production long-lived Services/Gateway readiness, non-Linux provider composition, and the real-provider cross-platform uninstall/upgrade recovery matrix. |
...
- complete production Runtime assignments, readiness, and Gateway bindings for
  release-backed OCI Tasks, long-lived Tool Services, and HTTP MCP. A trusted
  Linux ACL can explicitly compose the shared Box provider for Tool Tasks; the
  default remains unassigned and there is no fallback. The
  ...
  their reviewed provider is present; Use contract tests prove exact receipt-owned Service
