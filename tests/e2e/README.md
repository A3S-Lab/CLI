...
dedicated `a3s.activity.v3` `MessagePort`, and sends a bounded context proposal
...
fixture_root="$PWD/tests/fixtures/activity-document-browser"
web_assets="$PWD/../../apps/web/dist/workspace"
...
A3S_USE_INSTALL_DIR="$fixture_root" \
RUST_MIN_STACK=2097152 \
target/debug/a3s \
  --config "$fixture_root/config.acl" \
  --directory "$fixture_root/workspace" \
