...
fn find_downloaded_managed_srt(root: &Path) -> Result<PathBuf, String> {
    ...
    while let Some(directory) = stack.pop() {
        ...
        for entry in entries {
            ...
            let is_compatibility_bridge = path.strip_prefix(root).ok().is_some_and(|relative| {
                relative
                    .components()
                    .any(|component| component.as_os_str() == OsStr::new("release-compat"))
            });
            if path.ends_with(expected_suffix) && !is_compatibility_bridge {
...
mod tests {
    ...
    fn standalone_updater_accepts_a_self_consistent_future_sandbox_payload() {
        ...
        package["version"] = serde_json::Value::String("0.0.68".to_string());
        ...
        lock["packages"][""]["dependencies"][a3s_code_core::sandbox::srt::SRT_NPM_PACKAGE_NAME] =
            serde_json::Value::String("0.0.68".to_string());
    ...
    #[cfg(unix)]
    #[test]
    fn managed_srt_discovery_ignores_the_inert_self_update_bridge() {
        let tmp = TempDir::new("managed-srt-with-compat-bridge");
        write_managed_srt_support(&tmp.root);
        let bridge_root = tmp.path("release-compat");
        write_managed_srt_support(&bridge_root);

        let found = find_downloaded_managed_srt(&tmp.root).unwrap();

        assert_eq!(
            found,
            tmp.path(a3s::components::MANAGED_SRT_PAYLOAD_RELATIVE_ROOT)
        );
    }
