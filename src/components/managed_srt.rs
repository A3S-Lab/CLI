...
pub fn validate_managed_srt_payload(root: &Path) -> anyhow::Result<PathBuf> {
    ...
    validate_managed_srt_payload_structure(root)
}

/// Verify the package identity, dependency lock, and executable layout of an
/// SRT support tree without applying the release-tree digest.
///
/// Release consumers must normally call [`validate_managed_srt_payload`].
/// This narrower check exists for trusted staging tests that exercise archive
/// activation independently from the immutable release payload.
#[doc(hidden)]
pub fn validate_managed_srt_payload_structure(root: &Path) -> anyhow::Result<PathBuf> {
