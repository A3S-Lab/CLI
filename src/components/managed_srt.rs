...
impl ManagedSrtRuntime {
    ...
    /// Perform the explicit one-time Windows machine provisioning required by
    /// the native sandbox provider.
    ///
    /// This is never called during ordinary startup: it can create a local
    /// sandbox account and WFP rules and therefore must remain behind a
    /// user-invoked command and the operating system's UAC prompt.
    pub async fn setup_windows_machine(&self) -> anyhow::Result<()> {
        #[cfg(windows)]
        {
            let mut command = Command::new(&self.node);
            command
                .arg(windows_child_argument_path(&self.executable))
                .arg("windows-install")
                .current_dir(
                    self.executable
                        .parent()
                        .context("managed SRT executable has no parent directory")?,
                )
            ...
            let status = command
                .status()
                .await
                .context("failed to launch managed Windows sandbox setup")?;
            if !status.success() {
                bail!("managed Windows sandbox setup exited with {status}");
            }
            Ok(())
        }
        #[cfg(not(windows))]
        {
            bail!("managed machine setup is required only on Windows")
        }
    }
...
#[cfg(windows)]
fn windows_child_argument_path(path: &Path) -> PathBuf {
...
fn platform_sandbox_prerequisite_hint() -> &'static str {
    ...
    #[cfg(windows)]
    {
        "Windows requires the one-time elevated managed-sandbox machine setup; run `a3s code sandbox setup` from an interactive terminal"
    }
