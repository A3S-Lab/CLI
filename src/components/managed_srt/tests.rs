...
#[cfg(windows)]
#[test]
fn windows_child_paths_drop_extended_length_prefixes_for_node() {
    assert_eq!(
        windows_child_argument_path(Path::new(r"\\?\D:\managed\cli.js")),
        PathBuf::from(r"D:\managed\cli.js")
    );
    assert_eq!(
        windows_child_argument_path(Path::new(r"\\?\UNC\server\share\cli.js")),
    ...
    let output = sandbox
        .exec_command(
            "cat service-127/config/.env.variant-3 2>/dev/null || true; \
             cat outside-hardlink-profile-alias-with-a-deliberately-long-name-0000.txt \
             2>/dev/null || true; \
