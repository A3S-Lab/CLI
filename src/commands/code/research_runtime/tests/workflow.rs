...
fn deepresearch_cli_uses_the_shared_hard_safety_envelope() {
    ...
    let engine_limits = a3s_deep_research::engine::EngineLimits::default();
    ...
    assert_eq!(
        ...
        engine_limits.planned_retrieval_stage_timeout_ms
    );
    assert_eq!(
    ...
    assert_eq!(
        spec.retrieval_stage_budget_ms,
        a3s_deep_research::engine::DEFAULT_PLANNING_BOOTSTRAP_STAGE_TIMEOUT_MS
            + engine_limits.planned_retrieval_stage_timeout_ms
    );
    assert_eq!(
        spec.question_review_stage_budget_ms,
        2 * a3s_deep_research::engine::DEFAULT_REPORT_STAGE_TIMEOUT_MS
    );
...
fn deepresearch_cli_scope_is_query_agnostic_and_only_explicitly_selected() {
    ...
    let web_timeout = a3s_deep_research::engine::DeepResearchRequestLimits::for_evidence_scope(
        a3s_deep_research::engine::EvidenceScope::WebAndWorkspace,
    )
    .workflow_timeout_ms;
    let local_timeout = a3s_deep_research::engine::DeepResearchRequestLimits::for_evidence_scope(
    ...
    assert_eq!(local_wording["limits"]["timeoutMs"], web_timeout);
    assert_eq!(web_wording["input"]["evidence_scope"], "web_and_workspace");
    assert_eq!(web_wording["limits"]["timeoutMs"], web_timeout);
    assert_eq!(explicit_local["input"]["evidence_scope"], "local_only");
    assert_eq!(explicit_local["limits"]["timeoutMs"], local_timeout);
    assert_eq!(explicit_web["input"]["evidence_scope"], "web_and_workspace");
    assert_eq!(explicit_web["limits"]["timeoutMs"], web_timeout);
