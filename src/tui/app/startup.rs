//! Opt-in startup phase timing for the interactive Code host.
...
/// render, so a short delay is the explicit post-first-frame boundary.
...
impl StartupTrace {
    ...
    /// Record the time spent since the preceding checkpoint. Output is opt-in
    /// and deliberately contains only stable phase names and durations.
