...
pub(crate) fn loop_run_prompt_with_runtime(
    ...
    let os_directive = match runtime_mode {
        ...
        LoopRuntimeMode::OsAvailable => {
            "This loop has os_runtime disabled. Run it locally and do not claim an OS RemoteUI view exists unless a later explicit OS tool call returns one."
        }
        LoopRuntimeMode::LocalNoOs => {
            "OS is not signed in for this TUI session. Run the loop locally, but keep the same report/state artifacts. Do not claim an OS RemoteUI view exists; tell the user `/login` enables Runtime parallelism and RemoteUI."
        }
    };
...
impl App {
        ...
        if os_available && spec.os_runtime {
            self.push_line(
                &Style::new().fg(TN_GRAY).render(
                    "  OS connected: use A3S Runtime parallel workers; inspect them with /ps",
                ),
            );
    ...
    pub(crate) fn render_loop_panel(&self, panel: &LoopPanel) -> String {
        ...
        let left_w = (width / 2).clamp(34, 58).min(width.saturating_sub(28));
        ...
        left.push(pad_to(
            &Style::new()
                .fg(TN_GRAY)
                .render("  Enter/r run · a audit · l logs · p /ps · i init · Esc"),
            left_w,
        ));
        if panel.loops.is_empty() {
            ...
            for (idx, item) in panel.loops.iter().enumerate() {
                ...
                left.push(pad_to(&style.render(&truncate(&row, left_w)), left_w));
            }
        }
        while left.len() < h {
        ...
        if let Some(summary) = selected {
            ...
            if summary.audit.missing.is_empty() {
                right.push(pad_to(&Style::new().fg(TN_GREEN).render("  none"), right_w));
            } else {
...
mod tests {
    ...
    #[test]
    fn parse_loop_command_keeps_quick_loop_compatible() {
    ...
    fn parse_loop_command_requires_names_for_targeted_subcommands() {
        ...
        assert_eq!(
            ...
            LoopCommand::Dashboard,
            "help should open the dashboard instead of starting a quick loop"
        );
    }
...
"#,
    ...
    fn loop_run_prompt_requires_os_runtime_and_remoteui_when_available() {
        ...
        assert!(p.contains("visible through `/ps`"), "{p}");
