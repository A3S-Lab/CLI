...
/// A changed file in the `/git` panel: porcelain X/Y status plus path.
#[derive(Clone)]
...
/// Run a git subcommand in `workspace`, returning stdout (+ stderr on failure).
pub(crate) async fn run_git(workspace: String, args: Vec<String>) -> String {
...
pub(crate) async fn git_status_log(workspace: String) -> (Vec<GitFile>, Vec<String>) {
    let status = run_git(
        workspace.clone(),
        vec![
            "status".into(),
...
pub(crate) async fn git_diff_file(workspace: String, file: GitFile) -> Vec<String> {
