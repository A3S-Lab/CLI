...
make_fixture() {
    ...
    local release_repository=${7:-A3S-Lab/CLI}
...
printf '%s' \
    ...
    >"$legacy_list"
make_fixture 1.3.0 x86_64-unknown-linux-gnu 0 0 0 0 A3S-Lab/a3s
...
run_install latest "$transition_root/bin" "$transition_root/data"
[[ "$("$transition_root/bin/a3s" --version)" == 'a3s 1.3.0' ]] \
    || fail 'latest did not select the newer former-monorepo stable release'
assert_file "$transition_root/data/web/1.3.0/index.html"
...
run_install 1.3.0 "$explicit_transition_root/bin" "$explicit_transition_root/data"
