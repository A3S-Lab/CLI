...
| Memory (agent LLM extract) | `code exec` omitted LazyFileMemoryStore; streaming cancel aborted extract (`extract_calls=0`) | Pass after fix: CLI wires `.with_memory(LazyFileMemoryStore)`; Core streaming extract uses independent cancel token (`eb893fc9`, CLI pin `a4b7653`). Live WB-auto (`workbuddy/auto`) persisted preference `Prefer rustfmt nightly…` into workspace `.a3s/memory/index.json` with tags `llm`/`extracted` (ROOT `/tmp/a3s-wb-mem-extract2.X4Cp`); host reopen via `code memory list rustfmt`; no `~/.a3s/memory` contamination. Hermetic: `auto_mode_persists_llm_memory_extraction_into_workspace_store`; Core `scoped_streaming_memory_extraction_survives_turn_cancel`. |
...
| `$okf` / Use Knowledge | Host OKF + `$okf` skill + agent tool | Host Pass (fail-closed without package). **`$okf` skill** Pass: exec materializes builtin skills; live `Skill` → `OKF_SKILL_LOADED`. **`use_knowledge_search` in `code exec`** Pass: hermetic `atomic_scoped_registers_use_knowledge_search_when_okf_is_projected`; live WB-auto after signed OKF-only `acme/knowledge` → `UKS_HIT=yes`. **Multi-surface OKF** Pass on **Homebrew `a3s-use 0.3.12`**: ROOT `/tmp/a3s-uks-brew.RRIago` installed `a3s/mock-compose` → host `concepts/lifecycle.md`; live WB-auto (`A3S_USE_INSTALL_DIR=/opt/homebrew/bin`) → `tool: use_knowledge_search` / `UKS_COMPOSE=yes concepts/lifecycle.md` (`wb-compose.txt`). Hermetic: `healthy_okf_projects_knowledge_while_sibling_runtime_surfaces_reconcile`. crates.io: `a3s-use-core 0.2.10`, `a3s-use`/`extension 0.3.12`. Official Use-Registry still admits no OKF package by default. |
| Sandbox bash | `code sandbox status` + exec bash | Pass: macos-seatbelt ready; live `tool: bash` → `SANDBOX_PONG_42` |
| Use capability projection | Homebrew `a3s-use 0.3.12` (`ed1157b`/`920490b` formula); CLI crate pins `=0.3.12` / core `=0.2.10`; rejects Use `<0.3.0` as Broken (`host_protocol_requirement`) | Pass: brew binary + crates published from Use `5c18b74` / tag `v0.3.12`. Automated release job failed only on darwin-x86_64 independent-rebuild hash mismatch; primary archives published manually from run `34614783215` (arm64 rebuild matched). |
