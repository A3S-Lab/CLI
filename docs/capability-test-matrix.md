...
Scope: a3s-code TUI / Core **8.5.10** paths for **zvec-grep (BM25)**, **ReMe-like memory**,
...
| `$okf` / Use Knowledge | Host OKF + `$okf` skill + agent tool | Host Pass (fail-closed without package). **`$okf` skill** Pass (re-arm `OKF_SKILL_LOADED`). **`use_knowledge_search`** Pass: hermetic + live OKF-only. **Multi-surface OKF** Pass on **Homebrew `a3s-use 0.3.12`**: ROOT `/tmp/a3s-uks-brew.RRIago` still serves host `concepts/lifecycle.md` on re-arm; prior WB-auto `UKS_COMPOSE=yes`. crates.io: core `0.2.10`, use/extension `0.3.12`. |
| Sandbox bash | `code sandbox status` + exec bash | Pass; **re-arm** macos-seatbelt ready; `tool: bash` → `SANDBOX_PONG_REARM_42` |
| Use capability projection | Homebrew `a3s-use 0.3.12`; CLI pins `=0.3.12` / core `=0.2.10`; rejects Use `<0.3.0` | Pass; **re-arm** `/opt/homebrew/bin/a3s-use --version` → `0.3.12` (PATH prefers brew). |
...
**Pin note:** Published Core `8.5.10` is git rev
`3caa04dcc5b8d6960caeb3cb4120eab8eea87d62` (tag `v8.5.10`). This tree's CLI
`Cargo.toml` version-pins `=8.5.10` at that git rev (no `path = "../code/core"`).
