...
| `$okf` / Use Knowledge | Host OKF + `$okf` skill + agent tool | Host Pass (fail-closed without package). **`$okf` skill** Pass: exec materializes builtin skills; live `Skill` → `OKF_SKILL_LOADED`. **`use_knowledge_search` in `code exec`** Pass: hermetic `atomic_scoped_registers_use_knowledge_search_when_okf_is_projected`; live WB-auto after signed OKF-only `acme/knowledge` → `UKS_HIT=yes` (`concepts/package-lifecycle.md`). **Multi-surface OKF** Pass after Use `617803e` / release **`v0.3.12`** (`f2412b6`): healthy promoted OKF stays queryable while sibling Tool/MCP reconcile — hermetic `healthy_okf_projects_knowledge_while_sibling_runtime_surfaces_reconcile`; live WB-auto on installed `a3s/mock-compose` → `tool: use_knowledge_search` + `UKS_COMPOSE=yes` (debug `a3s-use` 0.3.12; Homebrew formula update pending release artifacts). Official Use-Registry still admits no OKF package by default. |
| Sandbox bash | `code sandbox status` + exec bash | Pass: macos-seatbelt ready; live `tool: bash` → `SANDBOX_PONG_42` |
| Use capability projection | Homebrew was `0.3.11` (lacks OKF sibling fix); Use tagged `v0.3.12`; CLI rejects Use `<0.3.0` as Broken (`host_protocol_requirement`) | Pass for protocol floor; **0.3.12 Homebrew formula pending** CI release artifacts |
