...
| `$okf` / Use Knowledge | Host OKF + `$okf` skill + agent tool | Host Pass (fail-closed without package). **`$okf` skill** Pass: exec materializes builtin skills; live `Skill` → `OKF_SKILL_LOADED`. **`use_knowledge_search` in `code exec`** Pass: hermetic `atomic_scoped_registers_use_knowledge_search_when_okf_is_projected`; live WB-auto (2026-09-11) after signed OKF-only `acme/knowledge` install into isolated `A3S_*_HOME` → `tool: use_knowledge_search` + `UKS_HIT=yes` citing `concepts/package-lifecycle.md` (CLI `a9515e4` admit). Note: multi-surface packages (e.g. Tool+MCP+OKF) keep OKF unpublished until the whole package is capability-ready — use an OKF-ready package for Knowledge-only proof; official Use-Registry currently admits Skill/Tool/MCP/UI demos without OKF. |
