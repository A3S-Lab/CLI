...
| `$okf` / Use Knowledge | Host OKF + `$okf` skill + agent tool | Host Pass (fail-closed without package). **`$okf` skill** Pass: exec materializes builtin skills; live `Skill` → `OKF_SKILL_LOADED`. **`use_knowledge_search` in `code exec`** Pass: hermetic `atomic_scoped_registers_use_knowledge_search_when_okf_is_projected`; live WB-auto after signed OKF-only `acme/knowledge` → `UKS_HIT=yes`. **Multi-surface OKF** Pass on debug `a3s-use 0.3.12`: ROOT `/tmp/a3s-uks-compose-0312.htJ6CA` installed `a3s/mock-compose` → host knowledge hit + live WB-auto `tool: use_knowledge_search` / `UKS_COMPOSE=yes concepts/lifecycle.md` (`wb-compose.txt`). Hermetic: `healthy_okf_projects_knowledge_while_sibling_runtime_surfaces_reconcile`. Use tag `v0.3.12` at `046b982` (release CI run `34606776654`); Homebrew formula pending artifacts. Official Use-Registry still admits no OKF package by default. |
| Sandbox bash | `code sandbox status` + exec bash | Pass: macos-seatbelt ready; live `tool: bash` → `SANDBOX_PONG_42` |
| Use capability projection | Homebrew was `0.3.11` (lacks OKF sibling fix); Use tagged `v0.3.12` at `046b982`; CLI rejects Use `<0.3.0` as Broken (`host_protocol_requirement`) | Pass for protocol floor; **0.3.12 Homebrew formula pending** CI release run `34606776654` |
