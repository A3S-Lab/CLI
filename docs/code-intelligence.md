...
A3S Code exposes one native Code Intelligence runtime to the agent and the TUI
workspace editor. It provides saved-file symbols, semantic
...
executables that match the workspace before starting `a3s code`:
...
are never published to the shared runtime. The TUI shows "saved version" when
an open buffer is dirty, and saving causes the shared
...
and cancellation; the TUI does not start language processes directly.
...
result metadata are enforced before a location reaches the TUI.
