...
**Pin:** host in `crates/cli`; Core **8.7.0** at git rev `c9e2650409be9e8eaf322e216e37314ac2029cda`.
...
| ID | Kind | Case | Expected | Status | Evidence / target |
| --- | --- | --- | --- | --- | --- |
| G1 | Effect. | Goal progress event-gated | Plan progress / wrong goal / user-turn / open ACCEPTANCE / maker phase / failing host-rechecked command / manual-only ACCEPTANCE do not mark goal complete; evidence fingerprints are skip-hints only; provider_error backs off, criterion_fail does not; verifier Plan chrome allows sandboxed bash + loop-dir writes; ACCEPTANCE `kind:command` and synthesized `test -f` for `kind:file_exists` attach Core reports; host `file_exists` matches `test -f` (rejects directories) and rejects workspace-escaping paths/outbound symlinks; when machine ACCEPTANCE exists on active loops (`running`/`retrying`/`paused`) Core emit requires a passing report for **each** such loop (not preset alone, not a completed-loop leftover, not a sibling/orphaned active loop alone); replacement `/goal` cancels prior/orphan active `goal-*` STATUS; goal session parallel admission host-clamped to ≤4 | covered | Host CLI: `unverified_first_iteration_continues_and_second_can_complete`; `only_the_current_extracted_goal_can_latch_achievement`; `user_turn_goal_events_cannot_close_the_host_goal_iteration`; `maker_phase_cannot_latch_achievement`; `open_acceptance_criteria_block_verifier_latch`; `manual_only_acceptance_cannot_latch_even_when_checked`; `host_rejects_manual_only_acceptance_contract`; `failing_command_criterion_blocks_verifier_latch_after_checkbox`; `passing_command_and_file_criteria_allow_verifier_latch`; `host_file_exists_rejects_directories_like_test_f`; `host_file_exists_rejects_paths_that_escape_workspace`; `persist_records_evidence_fingerprints_but_stale_fp_cannot_skip_latch_recheck`; `provider_error_backs_off_criterion_fail_does_not`; `continue_schedule_backs_off_provider_errors_not_criterion_misses`; `continue_schedule_persists_stall_class_into_runtime_state`; `goal_verify_plan_allows_sandboxed_bash_and_loop_dir_writes_only`; `start_goal_run_cancels_previous_loop_runtime_status`; `goal_parallel_admission_clamps_ultracode_budget_to_plan_window`; `goal_session_parallel_admission_clamps_only_when_goal_active`. Core (pin `c7e28eec…`, `verification::tests`): `shell_verification_report_attaches_for_goal_acceptance_commands`; `acceptance_file_exists_quotes_paths_with_spaces`; `acceptance_file_exists_skips_paths_that_escape_workspace`; `goal_emit_requires_acceptance_report_when_machine_acceptance_exists`; `stale_completed_loop_acceptance_does_not_authorize_or_pollute_active_goal`; `each_active_loop_needs_its_own_acceptance_report_before_goal_emit` |
...
| G4 | UX | Goal/loop meter | Footer authority matches run state; goal chip prefers ACCEPTANCE `n/m` over plan % | covered | `footer_medium_width_keeps_live_goal_before_optional_identity`; `footer_wide_width_keeps_all_optional_detail_after_mode_and_context`; `active_retrieval_precedes_goal_while_ready_retrieval_follows_it`; `criteria_progress_is_primary_completion_meter_not_plan_percent` |
...
| K1 | Contract. | Composer glyphs | `!` shell / `?` research exclusive priority; `$` skill mentions stay visible | covered | `composer_prompt_glyph_modes_are_exclusive_priority`; `composer_prompt_glyph_defaults_to_muted_arrow`; `dollar_mentions_select_enabled_skills_without_rewriting_the_visible_prompt` |
...
| G1 | Progress / wrong goal / user-turn / open ACCEPTANCE / maker phase / failing host-rechecked command / manual-only ACCEPTANCE ≠ goal complete; fingerprints ≠ latch skip; ACCEPTANCE command + file_exists attach Core reports; host file_exists rejects dirs and workspace-escaping paths; Core emit needs per-active-loop acceptance reports (orphan sibling alone insufficient); replacement `/goal` cancels prior/orphan active STATUS; goal parallel ≤4 host clamp; provider backoff vs criterion immediate continue | `unverified_first_iteration_continues_and_second_can_complete`; `only_the_current_extracted_goal_can_latch_achievement`; `user_turn_goal_events_cannot_close_the_host_goal_iteration`; `maker_phase_cannot_latch_achievement`; `open_acceptance_criteria_block_verifier_latch`; `manual_only_acceptance_cannot_latch_even_when_checked`; `host_rejects_manual_only_acceptance_contract`; `failing_command_criterion_blocks_verifier_latch_after_checkbox`; `passing_command_and_file_criteria_allow_verifier_latch`; `host_file_exists_rejects_directories_like_test_f`; `host_file_exists_rejects_paths_that_escape_workspace`; `persist_records_evidence_fingerprints_but_stale_fp_cannot_skip_latch_recheck`; `provider_error_backs_off_criterion_fail_does_not`; `continue_schedule_backs_off_provider_errors_not_criterion_misses`; `continue_schedule_persists_stall_class_into_runtime_state`; `goal_verify_plan_allows_sandboxed_bash_and_loop_dir_writes_only`; `shell_verification_report_attaches_for_goal_acceptance_commands`; `acceptance_file_exists_quotes_paths_with_spaces`; `acceptance_file_exists_skips_paths_that_escape_workspace`; `goal_emit_requires_acceptance_report_when_machine_acceptance_exists`; `stale_completed_loop_acceptance_does_not_authorize_or_pollute_active_goal`; `each_active_loop_needs_its_own_acceptance_report_before_goal_emit`; `start_goal_run_cancels_previous_loop_runtime_status`; `goal_parallel_admission_clamps_ultracode_budget_to_plan_window`; `goal_session_parallel_admission_clamps_only_when_goal_active` |
